const pokemon = Object.freeze([
    ...
    const moreThan1Type = pokemon.filter(val => val.types.length > 1);
    ...
    const pokemonIdsGreaterThan99 = pokemon.filter(val => val.id > 99).map(p => p.name)
    // console.log(pokemonIdsGreaterThan99)
    //#6 An array with just the names of the pokemon whose only type is poison
    const pokemonNamesTypePoison = pokemon.filter(val => val.types == 'poison').map(p => p.name)
    // console.log(pokemonNamesTypePoison)
    //#7 An array containing just the first type of all the pokemon whose second type is "flying"
    const secondTypeFlying = pokemon.filter(val => val.types[1] == 'flying').map(p => p.types[0])
    //console.log(secondTypeFlying)
    //#8 A count of the number of pokemon that are "normal" type
    const normalTypeCount = pokemon.filter(val => val.types.includes('normal')).length
